  ...
  create() {
    ...
    for (let s = 0; s < this.subdivisions; ++s) {
      ...
      for (let t = 0; t < triangles.length; ++t) {
        ...
        let t0 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3
        ))
        let t1 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3
        ))
        let t2 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3
        ))
        let t3 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3
        ))
